...
- Instead of greedily picking most likely word -> keep track of k most likely sequences
...
- `Luong Attention or Multiplicative Attention`:
  - uses dot product to measure similarity between 2 vectors
...
- Transorfmers:
  ...
  - sequence elements are NOT processed sequentially
    ...
    - easier to compute long-range patterns
...
- Structure:
  ...
  - All elements in sequence treated in parallel -> no notion of order => `positional encoding` added to input embeddings
    - Dense vector -> encodes position of each element in sequence
    - Use Embedding layer (in paper fixed positional encoding with sine and cosine functions)
...
- With:
  ...
  - $V \in \mathbb{R}^{n_v \times d_v}$: Value matrix (one row per value) (each row = vector of dimension $d_v$)
...
- Prior to calling scaled dot-product attention -> Q, K, V are transformed using linear transformations (time distributed dense layers without activation)
- `Number of heads` = number of linear transformations
  - each linear transformation applied to all elements in sequence
  - these linear transformations have learnable parameters
- Results of scaled dot-prodcut attention -> concatenated
...
These tensors passed seperately to scaled dot-product attention:
